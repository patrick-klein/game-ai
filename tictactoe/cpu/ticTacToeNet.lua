...
if net == nil then
    ...
    if false then
        ...
        net:add(nn.ReLU())
        --net:add(nn.Dropout(0.75))
        --net:add(nn.Linear(2048,1024))
        --net:add(nn.SoftPlus())        
        --net:add(nn.Dropout(0.05))
        ...
        --net:add(nn.MulConstant(0.01))       --ensures initial outputs are near zero
        --net:add(nn.HardTanh(-2,2))        --clamp output values within range
...
local numReplay = 512   --number of moves to remember
local numMoves = 512       --number of moves to train per episode
...
--loop through '' (remember one game and train over 'numMoves' moves)
